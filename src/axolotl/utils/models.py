...
class ModelLoader:
    ...
    def load_model(self) -> Tuple[PreTrainedModel, Optional[PeftConfig]]:
        ...
        if self.cfg.eagle_training:
            from axolotl.monkeypatch.eagle_utils import EagleAugmentedModel
            self.model = EagleAugmentedModel(
                model=self.model, 
                freeze_model=True,
                num_layers=self.cfg.num_eagle_layers,
                hidden_state_loss_coef=self.cfg.hidden_state_loss_coef,
                lm_loss_coef=self.cfg.lm_loss_coef,
            )

        # LOG.info(f"Sparse training: {self.cfg.sparse_training}")
        # LOG.info(f"Sparsity level: {self.cfg.sparsity_level}")
        # LOG.info(self.cfg)
        # # TODO: JAMES SPARSE MONKEYPATCH
        # if self.cfg.sparse_training is not None and self.cfg.sparse_training > 0:
        #     from transformers import LlamaForCausalLM
        #     assert isinstance(self.model, LlamaForCausalLM), "Only Llama models supported for sparse training for now"

        #     # TODO: update later with relevant hyperparams once I finalize
        #     LOG.info(f"Sparse training with sparsity level {self.cfg.sparsity_level}, using distributions in {self.cfg.histogram_path}")

        #     from axolotl.monkeypatch.sparse_utils import monkeypatch_sparse_forward
        #     monkeypatch_sparse_forward(self.model, self.cfg.histogram_path,self.cfg.sparsity_level, self.cfg.lookup_path)
